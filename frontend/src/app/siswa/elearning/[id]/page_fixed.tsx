...
export default function SiswaMapelDetailPage() {
	...
	const fetchTaskCountForMateri = useCallback(
		async (materiId: number) => {
			try {
				const response = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/elearning/materi/${materiId}/tugas?status=PUBLISHED`,
					{
						headers: { Authorization: `Bearer ${token}` },
					},
				);
		...
		<div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50 p-6">
			<div className="max-w-5xl mx-auto">
				...
				{!loading && materi.length > 0 && (
					<>
						<h2 className="text-2xl font-bold text-gray-900 mb-4">📚 Materi</h2>
				...
				{/* Empty State */}
				{!loading && materi.length === 0 && tugas.length === 0 && (
					<div className="text-center py-12 bg-white rounded-lg">
						<p className="text-gray-500 text-lg">
							Belum ada materi atau tugas untuk mata pelajaran ini
						</p>
					</div>
				)}
			</div>
