...
export default function GuruMateriDetailPage() {
		...
		try {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/elearning/rencana/${editingRencana?.id}`,
				{
					...
					body: JSON.stringify({ rencana: editRencanaText }),
				},
			);

			if (!response.ok) {
				showError("Gagal mengupdate rencana");
				return;
			}

			showSuccess("Rencana berhasil diupdate");
	...
	const handleDeleteRencana = (rencanaId: number) => {
		showConfirm({
			title: "Hapus Rencana",
			message: "Yakin ingin menghapus rencana pembelajaran ini?",
			onConfirm: async () => {
				try {
					const response = await fetch(
					...
					showSuccess("Rencana berhasil dihapus");
					fetchRencana();
				} catch (error) {
	...
	const handleDeleteKonten = (kontenId: number) => {
		showConfirm({
			title: "Hapus Konten",
	...
	const handleSaveTugas = async () => {
		...
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/elearning/tugas`,
				{
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const handleDeleteTugas = (tugasId: number) => {
		showConfirm({
			title: "Hapus Tugas",
	...
	const handlePublishTugas = async (tugasId: number) => {
		try {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/elearning/tugas/${tugasId}/publish`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${token}` },
				},
			...
			if (!response.ok) {
				showError("Gagal mempublikasikan tugas");
				return;
			}
	...
	const handleCloseTugas = async (tugasId: number) => {
		try {
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/elearning/tugas/${tugasId}/close`,
				{
					method: "POST",
					headers: { Authorization: `Bearer ${token}` },
				},
	...
	const handleUpdateStatus = async (
		...
		try {
			...
			if (newStatus === "PUBLISHED") {
				// Use POST /materi/:id/publish endpoint
				response = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/elearning/materi/${materiId}/publish`,
					{
						method: "POST",
						headers: {
							Authorization: `Bearer ${token}`,
				...
				// Use POST /materi/:id/close endpoint
				response = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/elearning/materi/${materiId}/close`,
					{
						method: "POST",
						headers: {
							Authorization: `Bearer ${token}`,
			...
			if (!response.ok) {
				const errorData = await response.json();
				showError(errorData.message || "Gagal mengubah status materi");
				return;
			}

			const data = await response.json();
			const updatedStatus = data.data?.status || data.status || newStatus;

			setMateriStatus(updatedStatus);
			if (materi) {
				setMateri({ ...materi, status: updatedStatus });
			}
			showSuccess(`Status materi berhasil diubah ke ${updatedStatus}`);
		} catch (error: any) {
								...
								{tugasList.map((tugas, idx) => (
									...
									>
										...
										<div className="flex gap-2">
											<Link
												href={`/guru/elearning/materi-detail/${materiId}/${
													tugas.tipe === "KUIS" ? "kuis-detail" : "tugas-detail"
												}/${tugas.id}`}
											...
											{tugas.status === "DRAFT" && (
												<button
													onClick={() => handlePublishTugas(tugas.id || 0)}
											...
											{tugas.status === "PUBLISHED" && (
												<button
													onClick={() => handleCloseTugas(tugas.id || 0)}
											...
											<button
												...
												className="ml-auto px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-sm rounded-lg transition-colors"
											>
												🗑️ Hapus
											</button>
										</div>
						...
						<div className="bg-white rounded-lg border border-gray-200 p-6">
							...
							<div className="grid grid-cols-3 gap-4">
								...
								<div className="bg-purple-50 rounded-lg p-4">
									<p className="text-gray-600 text-sm">Tugas & Kuis</p>
